...
/* ===== Country narrative description ===== */
function _buildCountryDescription(country, overall, rank, totalCountries) {
...
function renderPeaceBar() {
  ...
  const statusItems = statusKeys.filter(k => conflicts[k].length > 0).map(k => {
    ...
    return `<div class="peace-status-row">
      ...
      <span class="peace-countries">${names.join(', ')}</span>
      <span class="peace-dots">...</span>
...
function renderCountry() {
  ...
  const descriptionText = _buildCountryDescription(country, overall, rank, ranking.length);
  ...
  container.innerHTML = `
    ...
    ${countryImg}
    ${countryMapHtml}
    <div class="country-description">${descriptionText}</div>
...
function renderEconomicDashboard(country) {
  ...
  if (econ.currency_code) {
    ...
    const numLocale = I18n.getLang() || 'en';
    const fmtInt = v => v.toLocaleString(numLocale, {maximumFractionDigits: 0});
    const fmtDec = v => v.toLocaleString(numLocale, {minimumFractionDigits: 2, maximumFractionDigits: 2});
    const localGdpPc = isUSD ? '' : ` ≈ ${econ.currency_symbol} ${fmtInt(econ.gdp_per_capita * econ.usd_exchange)}`;
    currencyHtml = `
      <div class="currency-box">
        ...
        ${!isUSD ? `<div class="currency-rate">1 USD = ${econ.currency_symbol} ${fmtDec(econ.usd_exchange)}</div>` : ''}
        ${localGdpPc ? `<div class="currency-local-gdp">${I18n.t('econ.gdp_per_capita')}: $${fmtInt(econ.gdp_per_capita)}${localGdpPc}</div>` : ''}
      </div>`;
  }
